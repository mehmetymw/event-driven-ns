...
func TestNewNotification_ValidSMS(t *testing.T) {
	n, err := NewNotification(ChannelSMS, "+90500000000", "Hello", PriorityNormal, nil)

	require.NoError(t, err)
	assert.Equal(t, ChannelSMS, n.Channel)
	assert.Equal(t, "+90500000000", n.Recipient)
...
func TestNewNotification_ScheduledStatus(t *testing.T) {
	...
	n, err := NewNotification(ChannelSMS, "+90500000000", "Scheduled msg", PriorityNormal, &scheduledTime)
...
func TestNewNotification_InvalidChannel(t *testing.T) {
	_, err := NewNotification(Channel("fax"), "+90500000000", "Hello", PriorityNormal, nil)
...
func TestNewNotification_EmptyContent(t *testing.T) {
	_, err := NewNotification(ChannelSMS, "+90500000000", "", PriorityNormal, nil)
...
func TestNewNotification_SMSContentTooLong(t *testing.T) {
	...
	_, err := NewNotification(ChannelSMS, "+90500000000", string(longContent), PriorityNormal, nil)

	assert.ErrorIs(t, err, ErrContentTooLong)
}

func TestNewNotification_InvalidPriority(t *testing.T) {
	_, err := NewNotification(ChannelSMS, "+90500000000", "Hello", Priority("urgent"), nil)

	assert.ErrorIs(t, err, ErrInvalidPriority)
}

func TestNotification_Cancel(t *testing.T) {
	n, _ := NewNotification(ChannelSMS, "+90500000000", "Hello", PriorityNormal, nil)
